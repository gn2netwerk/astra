...
const DocsPopup = () => {
	...
	const kbData = astra_admin.astra_docs_data;
	const docsTrigger = () => {
		if ( kbData ) {
			setOpen(true);
		} else {
			window.open( "https://wpastra.com/docs/", '_blank' );
		}
	};
	return (
		<div>
			<button onClick={docsTrigger} className="text-xs sm:text-sm font-medium sm:leading-[0.875rem] text-slate-600">
