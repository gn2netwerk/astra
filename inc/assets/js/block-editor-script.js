...
function astra_onload_function() {
	...
	if( null !== titleBlock ) {
		...
		if( 'disabled' === wp.data.select( 'core/editor' ).getEditedPostAttribute( 'meta' )['site-post-title'] && ! titleVisibilityWrapper.classList.contains( 'invisible' ) ) {
		...
		titleVisibilityTriggerElement.addEventListener("click", function() {
			let metaTitleOptions = wp.data.select( 'core/editor' ).getEditedPostAttribute( 'meta' )['site-post-title'] || '';
